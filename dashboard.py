...
:root{color-scheme:light;--bg:#f6f6f4;--surface:#fcfcfb;--border:#e4e3df;--text:#0b0b0b;--text2:#52514e;--muted:#8a8984;
 --s1:#2a78d6;--base:#9a9993;--pos-bg:rgba(42,120,214,.16);--neg-bg:rgba(235,104,52,.18);--good:#008300;--bad:#c2410c;--warn:#b77900}
@media (prefers-color-scheme:dark){:root{color-scheme:dark;--bg:#111110;--surface:#1a1a19;--border:#2e2e2c;--text:#fff;--text2:#c3c2b7;--muted:#8d8c86;
 --s1:#3987e5;--base:#7a7973;--pos-bg:rgba(57,135,229,.25);--neg-bg:rgba(217,89,38,.28);--good:#4cb34c;--bad:#f07a4a;--warn:#e0a526}}
...
.legend{display:flex;gap:18px;font-size:13px;color:var(--text2);margin-bottom:8px}
...
<div class="card">
 <h2>Растеж на 100 € (портфейл, out-of-sample)</h2>
 <div class="legend"><span><i style="background:var(--s1)"></i>Модел</span><span><i style="background:var(--base)"></i>Купи и дръж</span></div>
 <div class="chartbox" id="eq"></div>
</div>

<div class="card"><h2>Резултати по активи</h2><table id="summary"></table></div>
...
document.getElementById("meta").textContent =
  `Обновено: ${D.generated_utc} · модел: ${D.model} · хоризонт: ${D.horizon} дни · цел: ${D.target[0]*100}–${D.target[1]*100}% годишно`;
...
const port = D.summary.find(r => r["Актив"] === "ПОРТФЕЙЛ" && r["Стратегия"] === "Модел");
const portBH = D.summary.find(r => r["Актив"] === "ПОРТФЕЙЛ" && r["Стратегия"] !== "Модел");
...
const cols = ["Годишна доходност","Средно на месец","Волатилност","Sharpe","Макс. спад","Печеливши месеци","Точност (посока)","В пазара"];
document.getElementById("summary").innerHTML =
  "<tr><th>Актив</th><th>Стратегия</th>" + cols.map(c => `<th>${c}</th>`).join("") + "</tr>" +
  D.summary.map(r => {
    const cls = (r["Стратегия"] !== "Модел" ? "bh " : "") + (r["Актив"] === "ПОРТФЕЙЛ" ? "port" : "");
...
// Линейна графика на чист SVG (без външни библиотеки)
(function () {
  const box = document.getElementById("eq"), E = D.equity, n = E.dates.length;
  if (!n) return;
  const W = box.clientWidth || 800, H = box.clientHeight || 300, m = {l: 58, r: 12, t: 10, b: 26};
  const all = E.model.concat(E.bh), lo = Math.min(...all), hi = Math.max(...all);
  const step = Math.pow(10, Math.floor(Math.log10((hi - lo) || 1))) / 2;
  ...
  for (let k = 0; k <= ticks; k++) {
    const v = y0 + (y1 - y0) * k / ticks, y = Y(v);
    g += `<line x1="${m.l}" x2="${W - m.r}" y1="${y}" y2="${y}" stroke="var(--border)"/>` +
         `<text x="${m.l - 8}" y="${y + 4}" text-anchor="end" fill="var(--text2)" font-size="12">${Math.round(v)} €</text>`;
  }
  ...
  box.innerHTML = `<svg width="${W}" height="${H}" role="img" aria-label="Растеж на 100 евро: модел срещу купи и дръж">${g}
    <path d="${path(E.bh)}" fill="none" stroke="var(--base)" stroke-width="2" stroke-linejoin="round"/>
    <path d="${path(E.model)}" fill="none" stroke="var(--s1)" stroke-width="2" stroke-linejoin="round"/>
    <line id="xh" y1="${m.t}" y2="${H - m.b}" stroke="var(--muted)" stroke-dasharray="3 3" visibility="hidden"/>
    <circle id="d1" r="4.5" fill="var(--s1)" stroke="var(--surface)" stroke-width="2" visibility="hidden"/>
    <circle id="d2" r="4.5" fill="var(--base)" stroke="var(--surface)" stroke-width="2" visibility="hidden"/>
    <rect x="${m.l}" y="${m.t}" width="${W - m.l - m.r}" height="${H - m.t - m.b}" fill="transparent" id="hit"/></svg>
    <div id="tip" style="position:absolute;pointer-events:none;display:none;background:var(--surface);border:1px solid var(--border);border-radius:8px;padding:8px 10px;font-size:13px;box-shadow:0 4px 14px rgba(0,0,0,.15)"></div>`;
  ...
  const show = ev => {
    ...
    [["#d1", E.model], ["#d2", E.bh]].forEach(([id, a]) => { const c = svg.querySelector(id);
      c.setAttribute("cx", x); c.setAttribute("cy", Y(a[i])); c.setAttribute("visibility", "visible"); });
    svg.querySelector("#xh").setAttribute("visibility", "visible");
    tip.innerHTML = `<div style="color:var(--text2)">${E.dates[i]}</div>
      <div><span style="color:var(--s1)">━</span> Модел: <b>${E.model[i].toFixed(0)} €</b></div>
      <div><span style="color:var(--base)">━</span> Купи и дръж: <b>${E.bh[i].toFixed(0)} €</b></div>`;
    tip.style.display = "block";
    tip.style.left = Math.min(x + 12, W - 190) + "px"; tip.style.top = "12px";
  };
  const hide = () => { tip.style.display = "none";
    svg.querySelectorAll("#xh,#d1,#d2").forEach(e => e.setAttribute("visibility", "hidden")); };
